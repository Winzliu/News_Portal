...
require '../../koneksi.php';
$id = $_GET['id'];
$namaKomentar = query("SELECT * FROM komentar WHERE id = $id")[0];
...
<body style="font-family: 'Noto serif', serif">
  ...
  <div class="d-flex bg-dark bg-opacity-50 position-absolute top-0 bottom-0 start-0 end-0">
    <div class="card m-auto" style="width: 32rem;">
      <div class="card-body">
        <h2 class="card-title text-center">Hapus Komentar</h2>
        <hr>
        <h4 class="card-subtitle my-5 text-center">Yakin Ingin Menghapus Komentar "<?= $namaKomentar['komentar'] ?>" ?
        </h4>
        <div class="text-center">
          <a href="index.php" class="btn btn-secondary me-5 fs-4">Batal</a>
          <a href="hapusKomentar.php?id=<?php echo $id; ?>" class="btn btn-danger fs-4">Hapus</a>
        </div>
      </div>
    </div>
  </div>
